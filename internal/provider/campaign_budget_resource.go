...
func (r *campaignBudgetResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Google Ads campaign budget. Delete uses CampaignBudgetService remove.", Attributes: mergeAttrs(idAttrs(), map[string]schema.Attribute{
		"name": schema.StringAttribute{Required: true}, "amount_micros": schema.Int64Attribute{Required: true}, "delivery_method": schema.StringAttribute{Optional: true, Computed: true, Description: "STANDARD or ACCELERATED where supported; defaults to STANDARD."}, "explicitly_shared": schema.BoolAttribute{Optional: true, Computed: true}, "status": schema.StringAttribute{Optional: true, Computed: true, Description: "ENABLED or REMOVED. Setting REMOVED is equivalent to delete."},
	})}
}
