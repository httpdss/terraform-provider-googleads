...
func (r *adGroupKeywordResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Google Ads ad group keyword criterion. Negative keywords are represented with criterion.negative=true. Delete uses AdGroupCriterionService remove.", Attributes: mergeAttrs(idAttrs(), map[string]schema.Attribute{"ad_group": schema.StringAttribute{Required: true}, "text": schema.StringAttribute{Required: true}, "match_type": schema.StringAttribute{Required: true}, "status": schema.StringAttribute{Optional: true, Computed: true}, "cpc_bid_micros": schema.Int64Attribute{Optional: true}, "negative": schema.BoolAttribute{Optional: true}})}
}
